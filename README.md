...
In the ***"Lv-1 Groups"*** property group panel, you can set each rowspan. This property is sparated into 17 sections by "===★" sign.   
...
Find the ***"1.===★Lv2 row span1======="*** field. And Enter "6" instead of default value 99. Then "Lv2" column's top 6 cells are merged.  
...
Find ***"◆Width(px) of Lv-2 col if 0 then hidden"***, and enter 0. It means the very left column becomes invisible. But now, you can also hide the Lv-2 column by the ***"Table Overall" → "◆Hide Lv2 Column"*** propery switch, and it's an easier way.  
...
If ***"Repeat Measure Name in dimension cols"*** is _"On"_, the table become very wide. But for now, I just wanted to explain the function. If the ***'◆Width(px) if 0...'*** property is 0, the column won't appear, anyway.  
...
Find ***"◆Width(px) of Meas Lavel if 0 then hidden"***, and enter 0. Then the measure name (label) column desappeared. But now, you can also hide the Measure Name by the ***"Table Overall" → "◆Hide Measure Name"*** propery switch, and it's an easier way. 
...
# Release note 
- January 8th, 2022 - qMaruPL have come to be able to hide indivisually each of the Lv2, Lv1, and Measure Name column by the "Table Overall" → "◆Hide ..." propery switch, without setting the width as 0. What's more, the "Row Break Cols" can be adjusted by the slider.  
...
(There were a great number of bugs before then, so I could't record them.)
